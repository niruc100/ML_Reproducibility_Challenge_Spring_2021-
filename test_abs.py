...
def test_abs(n_test,n_batch,n_steps,alpha,u,x_test):
    ...
    zeropad = nn.ZeroPad2d(height//2) # 4 times oversampling
    #zeropad = nn.ZeroPad2d(height) # 8 times oversampling
    ...
    for iters in pbar:
        ...
        #z = x_gt + uk         # without oversampling
        z = zeropad(x_gt + uk) # with oversampling
        ...
        for kx in range(n_steps):
            ...
            #z_est = x_est + uk + eps_tensor  # without oversampling
            z_est = zeropad(x_est + uk + eps_tensor) # with oversampling
            ...
            #x_grad = x_grad_complex[:,:,:,:,0] # without oversampling
            #x_grad = x_grad_complex[:,:, height//4:height//4+height, width//4 :width//4 +width, 0] # 2 times
            x_grad = x_grad_complex[:,:, height//2:height//2+height, width//2 :width//2 +width, 0] # 4 times
            #x_grad = x_grad_complex[:,:, height:height+height, width :width +width, 0] # 8 times

            
            x_est = x_est - alpha * x_grad
            x_est = torch.clamp(x_est, 0, 1)
 
# Articfacts that we removed from the code, because they slowed computation down
